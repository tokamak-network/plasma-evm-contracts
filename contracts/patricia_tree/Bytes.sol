...
library Bytes {
    ...
    // Checks if two `bytes memory` uintiables are equal. This is done using hashing,
    ...
    // Checks if two 'bytes memory' uintiables points to the same bytes array.
    ...
    function concat(bytes memory self, bytes memory other) internal pure returns (bytes memory) {
        ...
        (dest, dest2Len) = Memory.fromBytes(ret);
        dest2 = dest + src2Len;
